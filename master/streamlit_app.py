...
def show_chat_page():
    ...
    if user_id and chat_with:
        ...
        def display_messages():
            ...
            st.markdown(f"""
                <div style="max-height: 300px; overflow-y: scroll; border: 1px solid #ccc; padding: 10px; background-color: #808080;">
        ...
        if st.button("Send"):
            if new_message: # and safe(new_message):
                ...
                data["messages"][chat_with][user_id].append(f"{user_id}: {new_message}")
                save_data(data)
